...
sapdata<-read.csv("C:/Users/BREB/OneDrive/Documents/MSc Thesis/Thesis/DiabetesData/OutlierRemovedOverSampled/RFE-RF/Top5/Sap_SVM_Selected.csv")

#seedarray <- c(131,124,451,689,320,420,987,150,489,143,323,550,740,430,303,264,456,970,950,880)
seedarray <- c(640,950,880)
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:5], train.y=as.factor(training[,6]), 
                               kernel="radial",class.weights = wts,tunecontrol = tune.control(sampling = "cross"), ranges=list(cost=2^(-4:4), gamma=c(2^(-8:8)))))
...
write.csv(sapdata,"C:/Users/BREB/OneDrive/Documents/MSc Thesis/Thesis/DiabetesData/OutlierRemovedOverSampled/RFE-RF/Top5/Sap_SVM_Selected.csv")
